
#> your_namespace:v1.21.615/load/secondary
#
# @within	your_namespace:v1.21.615/load/main
#

# Python Datapack Template
scoreboard objectives add your_namespace.data dummy
tag Stoupy51 add convention.debug

# Check dependencies and wait for a player to connect (to get server version)
function your_namespace:v1.21.615/load/check_dependencies
function your_namespace:v1.21.615/load/valid_dependencies

