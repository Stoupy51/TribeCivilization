...
execute as @a[gamemode=survival,predicate=tribe_civilization:chance/0.005] at @s positioned over world_surface if entity @s[distance=..5] summon item_display run function tribe_civilization:falling_stars/summon

# Tick function for each falling star
execute as @e[tag=tribe_civilization.falling_star] run function tribe_civilization:falling_stars/entity_tick
